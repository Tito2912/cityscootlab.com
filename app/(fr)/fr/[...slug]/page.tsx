...
export async function generateStaticParams() {
  ...
  return all.filter((x) => x.slug[0] === 'fr' && x.slug.length > 1).map((x) => ({ slug: x.slug.slice(1) }));
}

export async function generateMetadata({ params }: { params: Promise<{ slug: string[] }> }): Promise<Metadata> {
  const { slug } = await params;
  const fullSlug = ['fr', ...slug];
  const meta = await getDocMetaByRouteSegments(fullSlug);
  if (!meta) return {};
...
export default async function FrCatchAllPage({ params }: { params: Promise<{ slug: string[] }> }) {
  const { slug } = await params;
  const fullSlug = ['fr', ...slug];
  const post = await getPostByRouteSegments(fullSlug);
